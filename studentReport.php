	...
	require_once 'php_action/connection.php';
 	...
 	class myPDF extends FPDF
 	{
 		function header()
 		{
			...
			$this->Cell(0,10,"Student Information",1,0,'C');
			$this->ln();
			$this->SetFont('times','',12);
			$this->SetTextColor(0,0,0);
			$this->Cell(176,10,"Student who can evaluate lecturer",0,0,'C');
			$this->ln(20);
 		}
 		function footer()
 		{
 		...
 		function headerTable()
 		{
 			...
 			$this->Cell(39,10,'Registration Number',1,0,'c');
 		...
 		{
 			$this->SetFont('Times','',12);
 			$stmt = $db->query("SELECT * FROM  student where action<>1");
              while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
              	$rs = $db->query("SELECT * FROM departement where dept_id='".$data->dept_id."'");
              		$record = $rs->fetch(PDO::FETCH_OBJ);
 				{
 					
		 			$this->Cell(39,10,$data->reg_no,1,0,'L');
		 			$this->Cell(25,10,$data->first_name,1,0,'L');
		 			$this->Cell(25,10,$data->last_name,1,0,'L');
		 			$this->Cell(42,10,$data->email,1,0,'L');
		 			$this->Cell(25,10,$data->phone_number,1,0,'L');
